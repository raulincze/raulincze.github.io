(function () {
	...
	function updateCategory() { // update selected category -> show green rectangle to the left of the category
		...
		for (var i = 0; i < this.sections.length; i++) {
			...
			if (top >= 0 && minTop > top) {
				minTop = top;
				closestElement = this.faqsCategories[i];
			}
			Util.toggleClass(this.faqsCategories[i], 'cd-faq__category-selected', false);
		}
		if (minTop !== Infinity) {
			Util.toggleClass(closestElement, 'cd-faq__category-selected', true);
		}
		else {
